...
class StringCalculatorTest < Minitest::Spec
  ...
  def test_add_split_comma_more_value
    assert_equal 10, @string_calc.add("1,2,3,4")
  end

  def test_add_split_lines
    assert_equal 6, @string_calc.add("1\n2,3")
  end

  def test_add_dynamic_delimiter
    assert_equal 3, @string_calc.add("//;\n1;2")
  end

  def test_add_exception_for_negative
    assert_raises RuntimeError, "Negatives not allowed: -1" do
      @string_calc.add("-1,2")
    end
  end

  def test_add_ignore_values_greater_1000
  ...
  def test_add_with_mixed_multiple_long_delimiters
    assert_equal 6, @string_calc.add("//[***][%]\n1***2%3")
  end
  ...
  def test_find_values_with_mixed_multiple_long_delimiters
    assert_equal %w[1 2 3], @string_calc.find_values("//[***][%]\n1***2%3")
  end
end